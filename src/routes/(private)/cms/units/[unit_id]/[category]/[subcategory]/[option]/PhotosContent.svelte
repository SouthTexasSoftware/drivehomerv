<script lang="ts">
  ...
  let photosStore = writable<PhotoDocument[]>([]);
  ...
  async function updateAndSortShowingArray() {
    photosSorted = false;
    ...
    for (let photoObject of unitObject.photos) {
      if (
        ...
        $photosStore.push(photoObject);
      }
    }

    photosStore.update((store) => {
      store = store.sort((photoA, photoB) => {
  ...
  function deletePhoto(photoDocument: PhotoDocument) {
    for (let photoIndex = 0; photoIndex < $photosStore.length; photoIndex++) {
      let photo = $photosStore[photoIndex];

      if (photo.id == photoDocument.id) {
        $photosStore.splice(photoIndex, 1);
        ...
        cmsStore.update((store) => {
          store.photosUpdated.deleted = photo.id;
          return store;
        });
  ...
  function updatePhotosIndexes() {
    ...
    photoElementsArray.forEach((photoElement, index) => {
    ...
    cmsStore.update((store) => {
      store.photosUpdated.unit_id = unitObject.id;
      store.photosUpdated.new_array = $photosStore;
      return store;
    });

    unitStore.update((store) => {
      let storeUnit = store.getUnit(unitObject.id);
      if (storeUnit) {
        storeUnit.photos = $photosStore;
        storeUnit.cms_edited = true;
      }
      return store;
    });
  }
...
{#if option}
  <div class="photos-option-container">
    ...
    <div class="sortable-photo-container" bind:this={sortableContainer}>
      {#key unitObject.cms_edited}
        {#if photosSorted}
          {#each $photosStore as photoObject (photoObject.id)}
            <div class="photo-container" data-id={photoObject.id}>
              <div
